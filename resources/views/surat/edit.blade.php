                        ...
                        <div class="pl-lg-4">
                                ...
                                <select name="kode_surat" id="kode_surat" class="form-control">
                                    ...
                                    @foreach (App\KodeSurat::all() as $key => $item)
                                        <option value="{{ $item->kode }}"
                                            {{ $surat->kode_surat == $item->kode ? 'selected' : '' }}>{{ $item->kode }} -
                                            {{ $item->nama }} {{ $item->uraian != '-' ? '- ' . $item->uraian : '' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <input class="form-control form-control-alternative" name="nama"
                                    placeholder="Masukkan Nama Surat" value="{{ $surat->nama }}">
                            </div>
                            <div class="form-group">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-8">
                                                <div class="form-group">
                                                    <textarea class="form-control" name="isi[]"
                                                        placeholder="{{ $isiSurat->jenis_isi == 1 ? 'Paragraf' : ($isiSurat->jenis_isi == 2 ? 'Kalimat' : ($isiSurat->jenis_isi == 3 ? 'Isian' : 'Subjudul')) }} ...">{{ $isiSurat->isi }}</textarea>
                                                </div>
                                                <div class="form-group">
                                                    <select name="isian[]" class="form-control"
                                                        style="{{ $isiSurat->jenis_isi != 3 ? 'display: none;' : '' }}">
                                                        <option value="">Pilih Isian</option>
                                                        @foreach ($isian as $key => $item)
                                                            <option value="{{ $item }}"
                                                                {{ $isiSurat->isian == $item ? 'selected' : '' }}>
                                            ...
                                            <div class="col-sm-4">
                                                @include('surat.side', [
                                                    'bantuan' =>
                                                        $isiSurat->jenis_isi == 1
                                                            ? url('img/bantuan-paragraf.png')
                                                            : ($isiSurat->jenis_isi == 2
                                                                ? url('img/bantuan-kalimat.png')
                                                                : url('img/bantuan-subjudul.png')),
                                                ])
                                            </div>
                                        </div>
                                    </div>
