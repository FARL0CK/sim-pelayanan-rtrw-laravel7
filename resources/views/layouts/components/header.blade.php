@php
    $menu = DB::table('artikel')
        ->where('menu', '!=', null)
        ->select('menu', 'submenu', 'sub_submenu')
        ->get()
        ->groupBy('menu');
@endphp
<header id="header">
    <div class="container-fluid d-flex justify-content-end" style="background-color: black">
        <div class="logo mr-auto d-none d-lg-block d-md-block">
            <!-- Uncomment below if you prefer to use an image logo -->
            <h2>
                <a href="{{ url('') }}">
                    <img src="{{ asset(Storage::url($desa->logo)) }}" alt="" class="img-fluid">
                    Sistem informasi pelayanan RW/RT Desa {{ $desa->nama_desa }} </a>
            </h2>
        </div>

        <!-- Navbar -->
        <nav class="nav-menu d-none d-lg-block" style="background-color: black">
            <ul>
                <li>
                        ...
                        @if (count($submenu->where('submenu', '!=', null)) > 0)
                            <ul>
                                @foreach ($submenu->where('submenu', '!=', null)->groupBy('submenu') as $submenu => $sub_submenu)
                                    <li
                                        class="{{ count($sub_submenu->where('submenu', '!=', null)->where('sub_submenu', '!=', null)) > 0 ? 'drop-down' : '' }}">
                                        <a
                                            href="/{{ Str::slug($menu) }}/{{ Str::slug($submenu) }}">{{ $submenu }}</a>
                                        @if (count($sub_submenu->where('submenu', '!=', null)->where('sub_submenu', '!=', null)) > 0)
                                            <ul>
                                                @foreach ($sub_submenu->where('submenu', '!=', null)->where('sub_submenu', '!=', null)->groupBy('sub_submenu') as $sub_submenu => $item)
                                                    <li><a
                                                            href="/{{ Str::slug($menu) }}/{{ Str::slug($submenu) }}/{{ Str::slug($sub_submenu) }}">{{ $sub_submenu }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
