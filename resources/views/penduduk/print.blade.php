...
@section('content')
    <div class="text-center mb-3" style="margin-top:20px">
        <h6 class="font-weight-bold">Data Penduduk RW 22</h6>
        <p>{{ tgl(date('Y-m-d')) }}</p>
    </div>
            ...
            @foreach ($penduduk as $anggota)
                @foreach ($anggota as $item)
                    <tr>
                        ...
                        <td style="vertical-align: middle; text-align: center;">
                            {{ tgl(date('Y-m-d', strtotime($item->tanggal_lahir))) }}</td>
                        <td style="vertical-align: middle; text-align: center;">
                            {{ date('Y') - date('Y', strtotime($item->tanggal_lahir)) }}</td>
                        ...
                        <td style="vertical-align: middle; text-align: center;">{{ $item->statusPerkawinan->nama ?? '-' }}
                        </td>
                        <td style="vertical-align: middle; text-align: center;">
                            {{ $item->statusHubunganDalamKeluarga->nama ?? '-' }}</td>
                        ...
                        <td style="vertical-align: middle; text-align: center;">{{ $item->statusPenduduk->nama ?? '-' }}
                        </td>
                    </tr>
                @endforeach
            @endforeach
